//require jquery-mobile.detect.js
var clicker = 'dblclick';
if(isMobile.any()) clicker = 'click';
    ...
    init: {
      ...
      desktop: function() {
        ...
        d.on(clicker, 'a.icon', function() {
		  ...
		  var id = 'js'+y.replace('#','');
		  if($("#"+id).length<1){	
          	$.getJSON($(y).attr('data'),function(data){
        ...
        d.on(clicker, 'div.window_top', function() {
          JQD.util.window_resize(this);
        });

        // Double click top bar icon to close, ala Windows OS.
        d.on(clicker, 'div.window_top img', function() {
...
function init(config){
	for(var i=0;i<config.modules.length;i++){
		$('#desktop').append(initWinSkin(config.modules[i]));
	}
	$("#bar_bottom").slideDown("slow");
	$("#desktop .icon").show("slow");
}

function initWinSkin(conf){
	var name=conf[0], title=conf[1], dataurl=conf[2], x=conf[3], y=conf[4];
	var win = initWinIcon(name, title, x, y);
	win += initWinBox(name, title, dataurl, name);
	return win;
}
function initWinIcon(name, title, x, y){
	var win = '<a class="abs icon" style="left:'+x+'px;top:'+y+'px;" href="#icon_dock_'+name+'">';
	win += '<img src="images/icons/'+name+'.png" />'+title+'</a>';
	return win;
}
function initWinBox(name, title, dataurl, icon){
	...
	win += '<div class="abs window_content">';
	win += '<div class="window_aside"><!-- left data --></div>';
	win += '<div class="window_main"><!-- content data --></div></div>';
